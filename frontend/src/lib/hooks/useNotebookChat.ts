...
export function useNotebookChat({ notebookId, sources, notes, contextSelections }: UseNotebookChatParams) {
  ...
  const sendMessage = useCallback(async (message: string, modelOverride?: string) => {
    ...
    try {
      ...
      let displayError = getApiErrorMessage(rawError, (key) => t(key as any), 'apiErrors.failedToSendMessage');

      // --- START INTERCEPTOR ---
      if (rawError.includes('No model configured')) {
        // We check if the UI is currently in Korean by looking at a known translated key
        if (t.common?.language === '언어' || t.common?.settings === '설정') {
          displayError = '채팅을 위한 기본 모델이 구성되지 않았습니다. 왼쪽 [설정] → [모델] 메뉴로 이동하여 AI 모델을 선택해주세요.';
        } else {
          displayError = 'No default model configured for chat. Please go to Settings → Models and select an AI model.';
        }
      }
      // --- END INTERCEPTOR ---
